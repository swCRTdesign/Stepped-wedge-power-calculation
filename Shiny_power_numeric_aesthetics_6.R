...
#1/6/19: After adding gamma and cleaning up some arguments, need to:
#Add a slider for gamma
#Add description
#Update descriptions for other parameters (=0 case)
#Check that additional warnings/messages don't interfere ('verbose' version?)

#2/27/19: This is an extension of the "with_gamma" app
#Making subjective organizational/aesthetic changes

#4/3: Making some structural changes to enable downloading the points in the plot - updated file name to 'version 2'

#4/5: After adding scrolling wellPanel, getting rid of the optional additional arguments panel; making rho a slider

#4/12: Adding ICC/CAC stuff

#5/20: changing some small stuff.  At this point, have not made any app-specific changes to the functions here.
...
#Also moving ICC/CAC conversion into app functions instead of doing them in the shiny server. (5/22: Actually, I don't think this would be helpful because a lot of things depend directly on the var components.)

#5/21: Haven't finished the ICC/CAC stuff yet, but made important changes to power plot for Binomial distribution, so moving to _6
...
#Temporary swDsn patch until next update of swCRTdesign
...
#Temporary patch until package is updated to accomodate n as a vector or matrix
#Update: inserted current version as of 2/6/19 (from swSim_swPwr_examples_draft.Rmd)
#5/8: Added warnings for rho=0, and updated binomial variation warning
swPwr.FlexN <- function (design, distn, n, mu0, mu1, tau, eta = 0, rho = 0, sigma, gamma = 0, 
                         alpha = 0.05, retDATA = FALSE) #EDIT: made default eta 0
{
  obs.per.cluster.per.time <- n
  if (length(n)>1){
    warning("When sample sizes are not uniform, power depends on order of clusters (see documentation).")
  }#added warning after 1/10 meeting
  if (rho != 0 & (tau == 0 | eta == 0)){#5/8: added this
  ...
  if (distn == "gaussian"){ 
    ...
    if (!missing(sigma)) 
      warning("sigma is not used when distn=binomial")#COMMENTED THIS OUT FOR APP - INCLUDE IN PACKAGE (5/22: nevermind)
    sigSq <- muBar * (1 - muBar)
    sigma <- NA
    if ((tau^2 + eta^2 + gamma^2) > sigSq) #5/8: updated
  ...
  Xmat.blk <- matrix(rep(as.vector(t(cbind(1, beta.blk))), 
                         I), ncol = J, byrow = TRUE)
  Xmat <- cbind(Xmat.blk, X.ij)
  if (length(n) == 1){#EDIT: added a condition
    #Added gamma here
    ...
    if ((is.vector(n) & length(n) > 1)) {
      #EDIT: n is a vector with one entry per cluster
      ...
      #EDIT: n is a matrix with one row per cluster and one column per time point
  ...
  for (i.indx in 1:length(I.rep)) {
    Xij.Xil.LIST_blk[[i.indx]] <- kronecker(diag(1, I.rep[i.indx]), 
                                            Xij.Xil.ARRAY[, , i.indx])
  }
  W.eta <- blkDiag(Xij.Xil.LIST_blk)
  Wmat <- Wmat.partial + as.matrix(W.eta)
  var.theta.WLS <- solve(t(Xmat) %*% solve(Wmat) %*% Xmat)["X.ij", 
                                                           "X.ij"]
  pwrWLS <- pnorm(abs(theta)/sqrt(var.theta.WLS) - qnorm(1 - 
                                                           alpha/2)) + pnorm(-abs(theta)/sqrt(var.theta.WLS) - qnorm(1 - 
                                                                                                                       alpha/2))
  rslt <- pwrWLS
  if (eta == 0 & gamma == 0 & length(n) == 1) {#EDIT: we can only calculate closed form when gamma=0 and n is an integer (original condition was only eta=0)
    X <- swDesign
    U <- sum(X)
    W <- sum(colSums(X)^2)
    V <- sum(rowSums(X)^2)
...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    sidebarPanel(
      ...
      sliderInput(inputId = "tau",
                  ...
                  min = 0,
                  max = 5,
                  value = 0,step=0.2),#Note: changed default to 0 for var components 2/15/19
      ...
      conditionalPanel(
        ...
        sliderInput(inputId = "icc",#Used to be numericInput
                    label = NULL,#"Rho",
                    min = 0,
                    max = 1,
                    value = 0,step=0.1),
        ...
        sliderInput(inputId = "cac",#Used to be numericInput
                    label = NULL,#"Rho",
                    min = 0,
                    max = 1,
                    value = 0,step=0.1)
      ...
      sliderInput(inputId = "sigma",
                  ...
                  min = 0,
                  max = 5,
                  value = 2,step=0.2)),#2/15/19: leaving this default at 1 -- changed to 2
      ...
      sliderInput(inputId = "rho",#Used to be numericInput
                   label = NULL,#"Rho",
                   min = -1,
                   max = 1,
                   value = 0,step=0.2)),
      
      #Used to have distribution here
      ...
      textInput(inputId = "tx.effect",
                label = NULL,"1")
      ),#end Wellpanel
      
      #Used to have plot options and parameterization here
      ...
      #Option to provide additional arguments (NOT design-related things)
      #checkboxInput("verboseON", 
       #             "Display additional optional arguments",
      #              value=FALSE
      #)
      
      #Turn on option to customize slider values (aka ranges)
      #checkboxInput("Customize","Customize slider values",value=FALSE)
    ),
    ...
    mainPanel(
      ...
      plotOutput(outputId = "distPlot"),#power curve#Tried withSpinner(,type=4)
      verbatimTextOutput("textoutput2"),
      plotOutput(outputId = "designPlot")#,#visualization of design
      
      #Other verbose arguments
      #conditionalPanel(
        #condition="input.verboseON",
        #h1("Other parameters"),
  ...
  tabPanel("Additional study design options",h1("Change other elements of the study design")
           ...
           fluidRow(
             ...
             mainPanel(
               ...
               #h3("Changing treatment assignments"),
               ...
               #h3("Changing number of clusters per wave"),
               ...
               #h3("Changing number of observations")
               ...
               #whichN when number of observations vary over CLUSTER (changed from time)
               conditionalPanel(
  ...
  tabPanel("Customize plots",h1("Control characteristics of the plots in first tab")
           ...
           fluidRow(
             ...
             mainPanel(
               ...
               h3("Power curve"),
               #h3("Select EITHER a minimum and maximum, or a range (distance from selected effect size)"),
               #Number of waves
               ...
               selectInput("effectsize_mode", label=NULL,#"Distribution",
               ...
               numericInput(inputId = "effectsize_distance",
                            ...
                            min = 0,
                            value = 0.1),
               #h5(tags$b("Distribution"),actionButton("info.distn", label=NULL,icon=icon("question-circle"),style='border-radius: 50%; padding: 1px; font-size:80%')),
               
               selectInput("plot_style", label="Style of plot",#"Distribution",
               ...
               selectInput("plot_extra_lines", label="Scale of design",#"Distribution",
  ...
  tabPanel("Help and References"#,h1("Manual and references go here")
           ...
           fluidRow(
             ...
             mainPanel(
               #h3("Test"),
               #h4("blah blah blah $$\\mu$$"),
               #tags$p("stuff here"),#This is plain-text!
               #tags$iframe(src="Manual_draft_pdf.pdf", width="900", height="600")
               h4("Report bugs not addressed below to: voldal@uw.edu"),
               #tags$p("Fill in citation suggestion here"),
               tags$p("Last updated: 5/22/2019"),
               ...
               tags$p("Based on the swCRTdesign R package by Jim Hughes and Navneet Hakhu."),
               
               includeHTML("manual_markdown.html")
             )
...
server <- function(input, output,session) {
  ...
  output$designPlot <- renderPlot({
    if(input$designplotON == TRUE){
      ...
      #if (input$plot_extra_lines == "sequence"){
        #design.plot.base
      #}
    ...
    try({
      ...
      GetPower <- function(effectsize){#,input.tau=input.tau,input.eta=input.eta,input.gamma=input.gamma,input.rho=input.rho
        ...
        if(input$distn == "binomial"){#mu0 is defined by user, and mu1 is mu0+effectsize
          #Added try to take care of boundary issues and var <= mu(1-mu) error
          #try({
      ...
      #Updated this condition after adding effectsize_mode
      ...
      #Problem: for binomial, can't just use effectsize (fixed above)
      #Also, need to weed out effect sizes that correspond to violating var < mu(1-mu)
      powerlist <- sapply(effectsizelist,GetPower)
      
      #plot(effectsizelist,powerlist,xlab="Effect size",ylab="Power",main="Power Curve")
      #abline(v=effectsize)
      #lines(effectsizelist,powerlist)#trying this out
      
      #effectsizelist <- effectsizelist[! is.na(powerlist)]
      #powerlist <- powerlist[! is.na(powerlist)]#For binomial case where there could be issues
  ...
  output$distPlot <- renderPlot({
    ...
    #data_for_download <- data.frame(effectsizelist,powerlist)
  ...
  output$textoutput <-  renderText({#was 'renderPrint'
    ...
    validate(
      ...
      #need(input$distn != "binomial" | input$gamma!= 0 | input$tau != 0 | input$eta != 0, "The selected variance components correspond to deterministic data; at least some need to be nonzero."),
      ...
      #need(input$rho >= -1 & input$rho <= 1, "Rho must be a numeral between -1 and 1."),
      ...
      #need(input$sigma == 0 | input$icc != 1,"When ICC is 1, sigma must be 0."),
  ...
  output$textoutput2 <-  renderText({
    ...
    #paste(variance_components_vector())
  })
