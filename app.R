...
#See LICENSE file for details
    # swCRTdesign shiny app for power calculations
    # Copyright (C) 2019  Navneet Hakhu, Patrick Heagerty, Jim Hughes, Emily Voldal
    # 
...
library(shiny)
library(swCRTdesign)#Built under version 2.2/3.0; Checked v. 3.1
library(ggplot2)#Updated for version 3.4.0

#Instructions for uploading GUI for future maintainers:
#Link: https://swcrtdesign.shinyapps.io/stepped_wedge_power_calculation/
#https://www.shinyapps.io
#rsconnect::deployApp('P:/Year_2/SWCRT RA/Shiny/Shiny_app_uploaded')#My 'testing' file
#rsconnect::deployApp('P:/Year_2/SWCRT RA/Shiny/stepped_wedge_power_calculation')#My current most polished version (Note: need to keep file name the same)
...
#Previously had swDsn here, but as of 10/25/19 switched to using the version from the swCRTdesign CRAN package v. 3.1
#Same code, but a couple extra warnings which are either already covered by the shiny warnings or are more precautionary
#If the messages in the console are aggrivating, could add silent option or argument indicating internal use to swDsn
...
#Body of the code is almost identical to swCRTdesign package v. 3.1; 
#biggest differences to look out for when making updates are in the inputs (different order and defaults) and warnings

swPwr.FlexN <- function (design, distn, n, mu0, mu1, H = NULL, tau, eta = 0, rho = 0, sigma, gamma = 0, 
                         alpha = 0.05, retDATA = FALSE) 
{
  #Last update: 10/25/2019, Emily Voldal
  #Last update: 9/16/2022, Jim Hughes
  #Last update: 10/7/2022, Emily Voldal
  #Last update: 1/12/2023, Emily Voldal (just design plot and help tags)
  ##########
  #Warnings and setting up variables
  ##########
  #Some of these warnings might be duplicates of the warnings done below directly in shiny.  
  #Keeping them for now as 'backup'
  if(!is.null(H) & (length(design$tx.effect.frac)>1 || design$tx.effect.frac<1)){#ECV 10/7 added the double 'or' to stop it from evaluating the second if it's a vector
...
#  X.ij <- as.vector(t(swDesign))
  ...
  if (length(n) == 1){
    ...
    for(i in 1:I){#for each of the I total clusters...
      ...
      #Line below works for this if there are no transition periods
...
#  var.theta.WLS <- solve(t(Xmat) %*% solve(Wmat) %*% Xmat)["X.ij", "X.ij"]
  ...
  if (eta == 0 & gamma == 0 & length(n) == 1 & is.null(H)) {#we can only calculate closed form when gamma=0 and n is an integer
    X <- swDesign
    U <- sum(X)
    W <- sum(colSums(X)^2)
    V <- sum(rowSums(X)^2)
...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    sidebarPanel(
      ...
      h5(tags$b("Treatment effect parameterization"),actionButton("info.tx_param", label=NULL,icon=icon("question-circle"),style='border-radius: 50%; padding: 1px; font-size:80%')),#,width='10%' padding:3px;
      radioButtons(inputId = "tx_param", label=NULL,
                   c("Immediate Treatment (IT)" = 1, "Exposure Time Indicator (ETI)" = 2)),
      
      conditionalPanel(#Exposure Time Indicator parameterization
      condition="input.tx_param == 2",
      h5(tags$b("ETI weighting vector (H) (comma-delimited)"),actionButton("info.Hvec", label=NULL,icon=icon("question-circle"),style='border-radius: 50%; padding: 1px; font-size:80%')),#,width='10%' padding:3px;
      textInput(inputId = "Hvec",
               label = NULL,"1")),
      ...
      conditionalPanel(#Random effects parameterization
      ...
      conditionalPanel(#ICC/CAC parameterization
  ...
  tabPanel("Help and References",
           ...
           fluidRow(
             ...
             mainPanel(
               ...
               tags$p("Last updated: 1/31/2023"),#Note: this date is the last update that would have affected the user end.
               ...
               tags$p("This program comes with ABSOLUTELY NO WARRANTY.  This is free software, and you are welcome to redistribute it under certain conditions.  Visit https://www.gnu.org/licenses/ for details."),
...
server <- function(input, output,session) {
  ...
  observeEvent(input$info.tx_param, {
    showModal(modalDialog(
      title = "Treatment effect parameterization",
      "The Immediate Treatment (IT) effect parameterization assumes that the treatment effect is immediate and constant regardless of exposure time. Exposure Time Indicator (ETI) parameterization 
      allows the treatment effect to vary as a function of exposure time. If you choose the ETI parameterization you will be asked to enter weights (H) to define the treatment effect, Psi = sum H(s)*theta(s) where theta(s)  
      is the treatment effect at exposure time s. For power calculations, only H and Psi, not the individual theta(s), need to be specified. See help tab for details.",
      easyclose = TRUE
    ))
  })
  observeEvent(input$info.Hvec, {
    showModal(modalDialog(
      title = "ETI weighting vector (H)",
      "Enter a list of numbers, separated by commas. This list gives the relative weights for a linear combination of the treatment effects at each exposure time. The
      overall treatment effect, Psi, is then defined as Psi = sum H(s)*theta(s) where theta(s) is the treatment effect at exposure time s (note: only H and the overall means under 
      the control and treatment conditions, not the individual theta(s), need to be specified). For equal weighting, 
	    the user may enter a single number; otherwise, the length of the list must be equal to the number of exposure times. If the elements of the list do not sum to 1.0 
	    then the weights will be re-normalized to sum to 1.0. Default is equal weighting which gives the average treatment effect over the exposure times.",
      easyClose = TRUE
    ))
  })
  observeEvent(input$info.mu1, {
    showModal(modalDialog(
      ...
      "Mean of the outcome in the treatment group. For the ETI model this is the control mean + Psi where Psi = sum H(s)*theta(s) (see help for H also)",
      easyClose = TRUE
    ))
  })
  ...
  observeEvent(input$info.clusters, {
    showModal(modalDialog(
      ...
      "The default ('constant') means that the same number of clusters switch to treatment in each sequence.  Alternatively ('varying'), the number of clusters that switch to treatment in each sequence can vary over time.",
      easyClose = TRUE
    ))
  })
  observeEvent(input$info.whichN, {
    showModal(modalDialog(
      ...
      "The default ('constant') means that there is the same number of observations in each cluster at each time point.  Alternatively ('varying by cluster'), 
      the number of observations can be different for each cluster (but be the same at every time point within each cluster).  Or ('varying by cluster and time') 
      the number of observations can be different for each cluster at each time point. For an IT model, one can specify a washout period (a period after introduction
      of the intervention where data are not used in the analysis) by selecting 'varying by cluster and time' and setting the number of observations equal to 0 for 
      the washout periods (note: for an ETI model specify washout periods by setting appropriate elements of the weighting vector, H, equal to 0).",
      easyClose = TRUE
    ))
  })
  ...
  output$designPlot <- renderPlot({
    if(input$designplotON == TRUE){
      ...
      design <- swDsn(clusters=design.vector,  extra.time=input$extra.time, all.ctl.time0=as.logical(input$all.ctl.time0))
      #For now, taking out , tx.effect=tx.effect.num from the design above so that squares in the plot are either control or treatment; not differentiating between treatment levels
      #Future extension: allow user to select a continuous scale for treatment which would reflect a fractional treatment effect
      ...
      design.plot.base <- ggplot(matrixdf, aes(x = Var2, y = Var1)) + 
        ...
        theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.box.background = element_rect(fill = NA,color=NA),legend.key = element_rect(fill = NA, colour = "black",linetype="solid",linewidth=1),plot.title=element_text(hjust=0.5))+
...
# Calculate weights for ETI model
  if (input$tx_param == 1) { 
    H = NULL
   } else {
    H = as.numeric(unlist(strsplit(input$Hvec,",")))
    if (length(H)==1) {H = rep(H,design$total.time-as.logical(input$all.ctl.time0))}
    H = H/sum(H)
   }
      
      GetPower <- function(effectsize){
        
        if(input$distn == "gaussian"){
          obj <- swPwr.FlexN(design=design,distn=input$distn,n=flexn,mu0=0,mu1=effectsize,H=H,tau=input.tau,eta=input.eta,sigma=input$sigma,gamma=input.gamma,rho=input.rho,alpha=input$alpha)
        
          }
        
        if(input$distn == "binomial"){#mu0 is defined by user, and mu1 is mu0+effectsize
          obj <- swPwr.FlexN(design=design,distn=input$distn,n=flexn,mu0=input$mu0,mu1=(input$mu0+effectsize),H=H,tau=input.tau,eta=input.eta,gamma=input.gamma,rho=input.rho,alpha=input$alpha,sigma=input$sigma)#removed ,sigma=input$sigma
  ...
  output$distPlot <- renderPlot({
    ...
    if(input$plotON == TRUE){#Especially with lines, power plots with only a few points can look really misleading.  Trying to prevent some of that here.
  ...
  output$textoutput <-  renderText({
    ...
    #Warning messages; goal is to prevent user from seeing R-generated errors which would be difficult to understand.
    #To accomplish this, most operations above are wrapped with try() and error messages are placed here instead.
    #Future extension: find a way to display some of the milder warnings (e.g. tx.effect.frac beyone [0,1])
    #Future extension: where applicable, make sure none of the info gets printed when there are critical warnings.  Currently, it's sometimes possible to have a serious issue with the design and still see the power plot beneath the warning message
    validate(
      #Main results
      ...
      need(input$tx_param == 1 | input$tx_param == 2, "Treatment effect parameterization must be 1 (IT) or 2 (ETI)"),
      need(all(as.numeric(unlist(strsplit(input$Hvec,","))) >= 0) & (length(as.numeric(unlist(strsplit(input$Hvec,",")))) == 1 | length(as.numeric(unlist(strsplit(input$Hvec,",")))) == input$waves+input$extra.time), "The ETI vector must be a number, or list of numbers separated by commas.  Length must be either 1 or equal to the number of exposure times."),
...
# Calculate H for ETI model    
    ...
    obj <- swPwr.FlexN(design=design,distn=input$distn,n=flexn,mu0=input$mu0,mu1=input$mu1,H=H,tau=input.tau,eta=input.eta,sigma=input$sigma,gamma=input.gamma,rho=input.rho,alpha=input$alpha)
    
    paste("Power:",round(obj,6))#Future extension: let user specify number of digits
  })
  ...
  #The main goal of these outputs is to help the user double-check that they specified the correct design.
  #Future extension: add a tab to see the conversion between random effects parameterization and ICC/CAC - e.g. conversion calculator
  ...
  output$textoutput2 <-  renderText({
    ...
    #Both sets of variance components
    if (input$variance_param == 1){
    if (input$eta == 0 & input$rho == 0){
      ...
      if ( ((input$tau^2+input$gamma^2)/(input$tau^2+input$gamma^2+input$sigma^2)) == 0){
    ...
    if(input$clusters != 1 | input$whichN != 1){
      ...
      ##################matrix unpacking, copied from power calculation section and added definition of d within statements.
	...
	g <- paste("Treatment effect parameterization:",ifelse(input$tx_param==1,"IT","ETI"))
    ...
    if (input$tx_param == 1) { 
      ...
      H = as.numeric(unlist(strsplit(input$Hvec,",")))
      if (length(H)==1) H = rep(H,input$waves+input$extra.time)
      H = H/sum(H)
      }
	
	h <- ifelse(input$tx_param==1,"",capture.output(cat("Renormalized weights for ETI parameterization:",H)))
  
    paste("Summary information (see help tab for details):",e,c,d,a,b,f,g,h,sep="\n")
